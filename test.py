...
def To_Generate_Key_Word(number):
    ...
    if wd == 0:
        ...
        w.to_file(f'/root/picture/Short Article: Top ten n-grams-WordCloud {xml_papersw[80][:-5]}.png')
    ...
    if wd == 1:
        ...
        w.generate_from_frequencies(dic)
        print("dfasdasdas")
        w.to_file(f'/root/picture/Top ten n-grams-WordCloud for {xml_papersw[80][:-5]}.png')
    #
    # i=0
    ...
    if wd == 0:
        ...
        plt.savefig(os.path.join(f'/root/picture/Short Article Trigram_figure_{xml_papersw[80][:-5]}'),
                    bbox_inches="tight")
        plt.close()
    ...
    if wd == 1:
        ...
        # Saving the figures in result path:
        # './img/pic-{}.png'.format(epoch + 1)
        plt.savefig(os.path.join(f'/root/picture/Regular Article Trigram_figure_{format(xml_papersw[80][:-5])}'),
                    bbox_inches="tight")
        plt.close()
...
def To_Generate_Location(number):
    ...
    for i, ppr in enumerate(GPE_top3):
        ...
        plt.savefig(f'/root/picture/Location_figure_{xml_inpuit_list[i]}', bbox_inches="tight")
        plt.close()
...
def To_Generate_Disease(number):
    ...
    for i, ppr in enumerate(disease_top3):
        ...
        plt.xticks(rotation=45, fontsize=20)
        plt.yticks(fontsize=20)
        plt.savefig(f'/root/picture/Disease_figure_{xml_inpuit_list[i]}', bbox_inches="tight")
        plt.close()
